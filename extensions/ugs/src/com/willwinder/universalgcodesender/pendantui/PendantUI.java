...
public class PendantUI implements UGSEventListener {
    ...
    public List<PendantURLBean> start() {
        ...
        URI baseUri = UriBuilder.fromUri("http://127.0.0.1/").port(port).build();

        server = JettyHttpContainerFactory.createServer(baseUri, false);
        // PCBs local override: Jersey's factory did not honour the URI host.
        // Set and verify the connector BEFORE server.start(), never after binding.
        for (Connector connector : server.getConnectors()) {
            if (!(connector instanceof ServerConnector)) {
                throw new IllegalStateException("Unsupported API connector; refusing to start");
            }
            ServerConnector network = (ServerConnector) connector;
            network.setHost("127.0.0.1");
            if (!"127.0.0.1".equals(network.getHost())) {
                throw new IllegalStateException("API connector is not loopback-only");
            }
        }
    ...
    public List<PendantURLBean> getUrlList() {
        ...
        out.add(new PendantURLBean(url, "Loopback only", bout.toByteArray()));
